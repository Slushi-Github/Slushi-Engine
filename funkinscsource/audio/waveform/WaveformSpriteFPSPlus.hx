...
class WaveformSpriteFPSPlus extends FlxSprite
{
  ...
  public override function new(x:Float, y:Float, _width:Int, _height:Int, _audioSource:FlxSound)
  {
    super(x, y);
  ...
  function updateWaveform()
  {
    ...
    if (audioSource._sound != null && audioSource._sound.__buffer != null)
    {
      ...
      wavData = waveformData(audioSource._sound.__buffer, bytes, st, et, waveformMultiply, wavData, Std.int(height));
    }
    ...
    for (i in 0...length)
    {
      ...
      pixels.fillRect(new Rectangle(hSize - (lmin + rmin), i * waveformDrawStep, (lmin + rmin) + (lmax + rmax), waveformDrawStep - waveformDrawNegativeSpace),
        waveformColor);
    }
